...
/** Granted one demo for one person. */
export async function POST(request: NextRequest) {
  ...
  const result = await callAdminRpc('grant_demo', { p_actor_id: actor.id, p_email: email, p_demo_slug: slug })
  return respond(request, result, `Granted ${slug} for ${email}`)
}
